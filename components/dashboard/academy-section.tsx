...
import { AcademyCatalogue } from "@/components/academy/academy-catalogue"
import { LiveSupportModal } from "@/components/modals/live-support-modal"
...
export function AcademySection() {
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500">
      ...
      {isSupportOpen && <LiveSupportModal onClose={() => setIsSupportOpen(false)} />}
    </div>
  )
}
...
function SupportCard({
  ...
  return (
    <div className="p-8 bg-slate-50 border border-slate-200 rounded-[40px] flex flex-col hover:bg-white hover:shadow-2xl hover:border-indigo-100 transition-all group">
      ...
      <BaseButton variant="secondary" size="sm" className="mt-auto self-start" onClick={onClick}>
        {action}
...
function PartnerDeal({
  ...
  return (
    <div className="p-6 bg-white border border-slate-200 rounded-3xl flex items-center justify-between group hover:border-indigo-200 transition-all cursor-pointer">
      ...
      <span className="px-2.5 py-1 bg-slate-100 text-[10px] font-bold text-slate-400 rounded-full uppercase tracking-wider">
        {tag}
      </span>
    </div>
  )
