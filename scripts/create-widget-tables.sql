...
CREATE TABLE IF NOT EXISTS widget_sessions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  widget_id UUID NOT NULL REFERENCES widget_chatbots(id) ON DELETE CASCADE,
  tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  -- Visitor info
...
CREATE INDEX IF NOT EXISTS idx_widget_chatbots_tenant ON widget_chatbots(tenant_id);
...
CREATE INDEX IF NOT EXISTS idx_widget_messages_widget ON widget_messages(widget_id);
...
CREATE POLICY "Users can view their tenant's chatbots" ON widget_chatbots
  FOR SELECT USING (
    tenant_id IN (
...
CREATE POLICY "Users can create chatbots for their tenant" ON widget_chatbots
...
CREATE POLICY "Users can update their tenant's chatbots" ON widget_chatbots
  FOR UPDATE USING (
    tenant_id IN (
      SELECT tenant_id FROM tenant_users WHERE user_id = auth.uid()
    )
...
-- RLS Policies for widget_sessions (need service role for inserts from widget)
CREATE POLICY "Users can view their tenant's sessions" ON widget_sessions
  FOR SELECT USING (
    tenant_id IN (
      SELECT tenant_id FROM tenant_users WHERE user_id = auth.uid()
    )
...
CREATE POLICY "Service role can insert sessions" ON widget_sessions
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Users can update their tenant's sessions" ON widget_sessions
  FOR UPDATE USING (
    tenant_id IN (
      SELECT tenant_id FROM tenant_users WHERE user_id = auth.uid()
    )
...
CREATE POLICY "Users can view their tenant's messages" ON widget_messages
  FOR SELECT USING (
    tenant_id IN (
      SELECT tenant_id FROM tenant_users WHERE user_id = auth.uid()
    )
...
CREATE POLICY "Service role can insert messages" ON widget_messages
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Users can update their tenant's messages" ON widget_messages
  FOR UPDATE USING (
    tenant_id IN (
      SELECT tenant_id FROM tenant_users WHERE user_id = auth.uid()
    )
